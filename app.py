...
from core.data import (
    ...
    build_graph_from_identifier_file,
)
...
with st.sidebar:
    ...
    genes_text = ""
    identifier_file_path = "identifiers.txt"
    ...
    # sumber 4: generic identifier file → STRING
    if source == "Identifier file → STRING":
        identifier_file_path = st.text_input(
            "Path file identifier",
            value=identifier_file_path,
            help=(
                "File teks lokal berisi gene symbols atau protein identifiers; "
                "satu per baris atau dipisah koma. Dataset pihak ketiga yang "
                "memerlukan lisensi tidak dibundel di repository ini."
            ),
        )
    ...
    if load_btn:
        try:
            with st.spinner("Mengambil data dari API dan membangun graf..."):
                ...
                elif source == "Identifier file → STRING":
                    G_new, meta = build_graph_from_identifier_file(
                        file_path=identifier_file_path,
                        source_name="identifier_file_to_string",
                        string_species=int(species),
                        required_score=int(required_score),
                        add_nodes=int(add_nodes),
                    )
                    label = f"IdentifierFile | file:{identifier_file_path} | taxon:{species} | score≥{required_score} | add_nodes:{add_nodes}"
                else:
