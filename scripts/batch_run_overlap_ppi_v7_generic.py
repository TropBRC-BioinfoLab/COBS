...
Graf dibangun langsung dari file identifier lokal menggunakan urllib + STRING API,
dengan fallback UniProt untuk accession yang tidak langsung dikenali STRING.

Contoh:
    python batch_run_overlap_ppi_v7_generic.py --input-file identifiers.txt --dataset-label my_dataset
    python batch_run_overlap_ppi_v7_generic.py --input-file identifiers.txt --dataset-label my_dataset --include-cobs
    python batch_run_overlap_ppi_v7_generic.py --input-file identifiers.txt --dataset-label my_dataset --include-keys slpa,kclique,cobs --include-cobs
"""
...
try:
    ...
    raise RuntimeError(
        ...
        "Pastikan script dijalankan dari salinan repository COBS dan seluruh dependensi "
        "pada requirements.txt sudah terpasang. "
...
def read_identifiers_txt(file_path: str | Path) -> list[str]:
    ...
    # Accept one identifier per line or comma/semicolon/tab-separated identifiers.
...
def main():
    ...
    ap.add_argument("--input-file", default="identifiers.txt", help="Path file teks identifier, satu per baris atau dipisah koma")
    ap.add_argument("--dataset-label", default="my_dataset", help="Label dataset untuk log dan output")
    ...
    ap.add_argument("--outdir", default="outputs_ppi_batch", help="Folder output")
    args = ap.parse_args()
